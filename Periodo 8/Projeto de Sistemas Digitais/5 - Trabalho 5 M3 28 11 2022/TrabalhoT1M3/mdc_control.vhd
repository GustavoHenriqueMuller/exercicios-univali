...
begin
	...
	p_NEXT_STATE: process(r_STATE, i_GO, i_X_EQ_Y, i_X_LT_Y) begin
		case (r_STATE) is
			...
			when s1 => if (i_GO = '1') then w_NEXT_STATE <= s2; else w_NEXT_STATE <= s1; end if;
			when s2 => w_NEXT_STATE <= s3;
			when s3 => w_NEXT_STATE <= s4;
			when s4 => if (i_X_EQ_Y = '0') then w_NEXT_STATE <= s5; else w_NEXT_STATE <= s9; end if;
			when s5 => if (i_X_LT_Y = '1') then w_NEXT_STATE <= s6; else w_NEXT_STATE <= s7; end if;
